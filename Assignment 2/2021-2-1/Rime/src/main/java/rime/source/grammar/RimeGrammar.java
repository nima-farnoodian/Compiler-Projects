...
public final class RimeGrammar extends Grammar {
	...
	private final rule emptyList = longest(
		seq(str("list"), L_PAREN, primitiveType, R_PAREN),
		seq(L_BRACKET, primitiveType, R_BRACKET)
	).push($ -> new EmptyList(new ArrayList<>(), $.$0()));
	
	private final rule emptySet = seq(str("set"), L_PAREN, primitiveType, R_PAREN)
		.push($ -> new EmptySet(new ArrayList<>(), $.$0()));
	
	private final rule emptyDict = seq(word("dict"), L_PAREN, primitiveType, COMMA, primitiveType, R_PAREN)
		.push($ -> new EmptyDict(new ArrayList<>(), $.$0(), $.$1()));
	...
	)).push($ -> new IfStatement($.$0(), $.$1()));
	
	private final rule elseIfStatement = seq(kw_else, ifStatement);
	
	private final rule elseStatement = lazy(() -> seq(
		kw_else,
		L_BRACE,
		this.statement,
		R_BRACE
	)).push($ -> new ElseStatement($.$0()));
	...
	public final rule entryPoint = seq(
		kw_proc,
		kw_main,
		seq(L_PAREN, str("[string]: args"), R_PAREN),
		L_BRACE,
		statement,
		R_BRACE
